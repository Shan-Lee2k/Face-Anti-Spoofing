def get_backbone(architecture_name, 
    ...
    if architecture_name == 'MobilenetV2':
        ...
        mode_large = 'large'
        mode_small = 'small'
        out_feature = 512 #256 Change feature size here
        model = MobileNetV3_Custom(pretrained=pretrained, mode = mode_large, out_feature= out_feature, drop_rate=0.3) #small
        if pretrained is None:
            print(f"Changed backbone of static method: MobilenetV3_{mode_large} --- (NO PRE-TRAINED)")
        else:
            print(f"Changed backbone of static method: MobilenetV3_{mode_large}--- (PRE-TRAINED)")
        #feature_size = 1280
        #feature_size = 256
